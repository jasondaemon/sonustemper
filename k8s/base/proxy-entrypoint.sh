...
if [ -f /etc/nginx/templates/nginx.conf.template ]; then
  ...
  sed -i "s/__PROXY_UPSTREAM_HOST__/${esc_host}/g" /etc/nginx/conf.d/default.conf
  sed -i "s/__PROXY_UPSTREAM_PORT__/${esc_port}/g" /etc/nginx/conf.d/default.conf
...
for i in $(seq 1 30); do
  ...
  echo "[proxy] waiting for DNS of ${PROXY_UPSTREAM_HOST} (attempt $i)..."
  sleep 1
done
